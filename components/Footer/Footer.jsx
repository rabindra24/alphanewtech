...
const Footer = () => {
  return (
    <div className=" relative z-10">
      <div className="grid md:grid-cols-4 sm:grid-cols-2 grid-cols-1  relative  mx-auto w-full border-[1px] border-black px-5">
        
        {FooterLinks.map(({ title, links, social }) => (
          <div className="border-[1px] border-black ">
            ...
            <div className="border-t-[1px] border-black w-full p-3 flex flex-col space-y-2 text-md text-gray-400 font-thin text-sm">
              {links.map((item) => {
                if (item.link) {
                  return (
                    <Link href={`${item.link}`} className="list-none">
                      {item.title}{" "}
                    </Link>
                  );
                } else {
